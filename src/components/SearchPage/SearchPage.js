...
function SearchPage(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=9&type=video&q=${searchQuery}&safeSearch=none&key=`
      )
      .then((response) => {
  ...
  async function createChannelRow(channel) {
    ...
    const response = await axios.get(
      `https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${channelId}&key=`
    );
  ...
  async function createVideoRows(videos) {
    ...
    for (const video of videos) {
      ...
      const response = await axios.get(
        `https://www.googleapis.com/youtube/v3/videos?part=statistics%2C%20snippet&id=${videoId}&key=`
      );
